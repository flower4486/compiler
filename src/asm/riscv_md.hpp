...
class RiscvDesc : public MachineDesc {
  ...
  private:
    ...
    void emitTac(tac::Tac *);//翻译一般TAC指令
    // translates a LoadImm4 TAC into assembly instructions
    void emitLoadImm4Tac(tac::Tac *);//翻译加载立即数的TAC指令
    // translates a Unary TAC into assembly instructions
    void emitUnaryTac(RiscvInstr::OpCode, tac::Tac *);//翻译一元运算TAC指令
    void emitAllocTac(tac::Tac *t);//翻译内存分配TAC指令
    void emitCallTac(RiscvInstr::OpCode, tac::Tac *);//翻译函数调用TAC指令
    void emitPushTac(tac::Tac *);//翻译入栈TAC指令
    void emitPushTac1(tac::Tac *);
    void emitLaTac(RiscvInstr::OpCode,tac::Tac *t);//翻译计算地址TAC指令
    void emitLwTac(RiscvInstr::OpCode,tac::Tac *t);//翻译加载数据TAC指令
    void emitSwTac(RiscvInstr::OpCode,tac::Tac *t);//翻译存储数据TAC指令
    // translates a Binary TAC into assembly instructions
    void emitBinaryTac(RiscvInstr::OpCode, tac::Tac *);//翻译二元运算TAC指令
    //STEP4
    void emitAssignTac(RiscvInstr::OpCode, tac::Tac *);//翻译赋值TAC指令
