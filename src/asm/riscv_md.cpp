...
RiscvInstr *RiscvDesc::prepareSingleChain(BasicBlock *b, FlowGraph *g) {
    ...
    for (Tac *t = b->tac_chain; t != NULL; t = t->next)
        emitTac(t);
    //后面这一串，都是基本快的结束指令，在这里实际操作栈帧，用到了之前保存的变量
    switch (b->end_kind) {
...
void RiscvDesc::emitBinaryTac(RiscvInstr::OpCode op, Tac *t) {
    ...
    //为什么要把这两个元操作数加入到活跃性表中，因为给op1分配的时候，可能把op2顶掉
...
void RiscvDesc::emitFuncty(Functy f) {
    ...
    //分析活跃性，目的是分析在当前基本块结束之后，还有哪些变量是活跃的。比如在这个基本快的
    //liveout_set中有一个t1变量，那么我们就可以知道，下一个基本块八成还是要用到他，所以
    //离开这个BLOCK时，得保存一下他。
    //为什么要这样搞呢？因为我们这个寄存器分配算法时针对基本块内部的，所以切换基本快的时候要
    //保存还要用到的。
    g->analyzeLiveness(); // computes LiveOut set of the basic blocks
    //第一个循环就是为了活跃性，遍历一遍基本块把他们的liveout_set放到栈帧中
    ...
    for (FlowGraph::iterator it = g->begin(); it != g->end(); ++it) {
        ...
        b->instr_chain = prepareSingleChain(b, g);//代码生成的主要部分
        //上面这个函数最终返回一个riscv指令链，节点是TAC
        //可以进行一些优化
        simplePeephole((RiscvInstr *)b->instr_chain);
        b->mark = 0; // clears the marks (for the next step)
    }
    //循环结束后最终得到一个riscv指令链，节点是TAC
    ...
    emitProlog(f->entry, _frame->getStackFrameSize());//目标代码输出的函数
    ...
    for (FlowGraph::iterator it = g->begin(); it != g->end(); ++it)
        emitTrace(*it, g);//把所有的指令一个个emit出来
}
